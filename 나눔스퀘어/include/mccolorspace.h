...
static const ColorSpaceBaseInfo_t  g_ColorSpaceBaseInfoMap[] =
{
    ...
    {FOURCC_BGRA32F,	CS_FORMAT_RGB,  32,	    CHROMA444,	        128,	        1,              "BGRA32F"       ,"BGRAFP" },
    ...
    {FOURCC_010P,	    CS_FORMAT_YUV,  10,	    CHROMA420,	        24,	            2,              "010P"	        ,"" },
    {FOURCC_P010,	    CS_FORMAT_YUV,  10,	    CHROMA420,	        24,	            2,              "P010"	        ,"" },
    {FOURCC_P016,	    CS_FORMAT_YUV,  16,	    CHROMA420,	        24,	            2,              "P016"	        ,"" },
    {FOURCC_210P,	    CS_FORMAT_YUV,  10,	    CHROMA422,	        32,	            2,              "210P"	        ,"" },
    ...
    {FOURCC_PLAYUV16,	    CS_FORMAT_YUV,  16,	    CHROMA444,	        64,	            1,              "PLAYUV16"      ,"PL_AYUV16" },
    ...
    {FOURCC_A64L,	    CS_FORMAT_RGB,  16,	    CHROMA444,	        64,	            1,              "A64L"	        ,"" },
...
*/
const static __inline uint8_t* get_name(uint32_t fourcc)
{
    int32_t i;
...
*/
static __inline uint32_t get_fourcc(const uint8_t* name)
{
    int32_t i;
    for (i = 0; i < g_ColorSpaceBaseInfoMapSize; i++)
    {
...
/** @brief Call this function to get type of color space (RGB, YUV, XYZ) by fourcc.
	 ...
	 */
static __inline int32_t get_cs_type(uint32_t fourcc)
{
    int32_t i;
    for(i = 0; i < g_ColorSpaceBaseInfoMapSize; i++)
        if(g_ColorSpaceBaseInfoMap[i].fourcc == fourcc)
...
/** @brief Call this function to get bit depth by fourcc.
	 ...
	 */
static __inline int32_t get_bit_depth(uint32_t fourcc)
{
    int32_t i;
    for(i = 0; i < g_ColorSpaceBaseInfoMapSize; i++)
        if(g_ColorSpaceBaseInfoMap[i].fourcc == fourcc)
...
/** @brief Call this function to get chroma format by fourcc.
	 ...
	 */
static __inline int32_t get_chroma_format(uint32_t fourcc)
{
    int32_t i;
    for(i = 0; i < g_ColorSpaceBaseInfoMapSize; i++)
        if(g_ColorSpaceBaseInfoMap[i].fourcc == fourcc)
...
/** @brief Call this function to count of bits per pixel by fourcc.
     ...
     *         0 if FourCC format is unsupported
	 */
static __inline uint32_t get_bits_per_pixel(int32_t fourcc)
{
    int32_t i;
    for(i = 0; i < g_ColorSpaceBaseInfoMapSize; i++)
...
/** @brief Call this function to get number of pixels per macropixel.
     ...
     *         0 if FourCC format is unsupported
	 */
static __inline uint32_t get_pixels_per_macropixel(int32_t fourcc)
{
    int32_t i;
    for(i = 0; i < g_ColorSpaceBaseInfoMapSize; i++)
        if(g_ColorSpaceBaseInfoMap[i].fourcc == (uint32_t) fourcc)
...
/** @brief Call this function to check color space frame parameters.
     ...
     *         1 if not
	 */
static __inline int32_t check_frame_colorspace_param(uint32_t width, uint32_t height, uint32_t fourcc)
{
    uint32_t pixels;
    int32_t chroma_format;
...
/** @brief Call this function to get color space frame information.
     ...
     *         1 if not
	 */
static __inline int32_t get_frame_colorspace_info(frame_colorspace_info_tt *cs_info, uint32_t width, uint32_t height, uint32_t fourcc, int32_t stride)
{
    int32_t line_offset;
    if(!cs_info)
        return 1;
    ...
    case FOURCC_IYUV:
        ...
        cs_info->plane_width[1] = cs_info->plane_width[2] = width / 2;
        break;
    ...
    case FOURCC_YV24:
        ...
        cs_info->planes = 3;
        cs_info->plane_height[1] = cs_info->plane_height[2] = height;
        cs_info->plane_width[1] = cs_info->plane_width[2] = width;
        break;
    ...
    case FOURCC_YUV9:
        ...
        cs_info->plane_width[1] = cs_info->plane_width[2] = width / 4;
        break;
    ...
    case FOURCC_PLAYUV16:
        cs_info->stride[0] = cs_info->stride[1] = cs_info->stride[2] = cs_info->stride[3] = stride ? stride : width * 2;
        cs_info->plane_offset[1] = cs_info->stride[0] * height;
        cs_info->plane_offset[2] = cs_info->plane_offset[1] + cs_info->stride[1] * height;
        cs_info->plane_offset[3] = cs_info->plane_offset[2] + cs_info->stride[2] * height;
        cs_info->frame_size = abs(cs_info->stride[0]) * height * 4;
        cs_info->planes = 4;
        cs_info->plane_height[1] = cs_info->plane_height[2] = cs_info->plane_height[3] = height;
        cs_info->plane_width[1] = cs_info->plane_width[2] = cs_info->plane_width[3] = width;
        break;
...
/** @brief Call this function to fill @ref frame_tt structure from @ref frame_colorspace_info_tt.
     ...
     *         1 if not
	 */
static __inline int32_t fill_frame_from_colorspace_info(const frame_colorspace_info_tt *cs_info, uint8_t *buffer, frame_tt *frame_tt_out)
{
    uint32_t i;
...
/** @brief Call this function to get offsets to the position for each plane in bytes.
     ...
     *         1 if not
	 */
static __inline int32_t get_plane_offsets_in_bytes(int32_t *offset, uint32_t x, uint32_t y, uint32_t fourcc, int32_t stride)
{
    int32_t line_offset;
    if(!offset)
        return 1;
    ...
    case FOURCC_PLAYUV16:
        offset[0] = offset[1] = offset[2] = offset[3] = x*2 + stride*y;
        break;
...
/** @brief Call this function to flip color space frame.
     ...
     * @param[in] cs_info pointer to @ref frame_colorspace_info_tt structure
     * @param[out] cs_info pointer to @ref frame_colorspace_info_tt structure
	 * @return 0 if the pointer of frame_colorspace_info_tt is correct\n
     *         1 if not
	 */
static __inline int32_t flip_colorspace(frame_colorspace_info_tt *cs_info)
{
    int8_t i;
    if(!cs_info)
        return 1;
...
*/
static __inline int32_t crop_frame_colorspace_info(frame_colorspace_info_tt *crop_cs_info, uint32_t pixels_number_left, uint32_t pixels_number_top, uint32_t pixels_number_right, uint32_t pixels_number_bottom)
{
    int32_t offset[4];
    int32_t error;
    ...
    for(i = 0; i < crop_cs_info->planes; i++)
    {
        ...
        crop_cs_info->plane_width[i] = cs_info_crop.plane_width[i];
    }
