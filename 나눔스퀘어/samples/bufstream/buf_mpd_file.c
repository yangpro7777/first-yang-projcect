...
bufstream_tt *open_file_buf_mpd(const char *bs_filename, uint32_t bufsize, void (*DisplayError)(char *txt));
int32_t init_file_buf_mpd(bufstream_tt *bs, const char *bs_filename, uint32_t bufsize, void (*DisplayError)(char *txt));
...
bufstream_tt * open_buf_mpd_file(const char * filename, uint32_t chunksize, void (*DisplayError)(char *txt))
{
    ...
    if(bs)
    {
        if(BS_OK != init_file_buf_mpd(bs, temp_filename, chunksize, DisplayError))
        {
            free(bs);
            bs = NULL;
            return bs;
...
static uint32_t usable_bytes_mpd(bufstream_tt *bs)
{
...
static uint8_t *request_mpd(bufstream_tt *bs, uint32_t numbytes)
{
    struct impl_stream* p = bs->Buf_IO_struct;
...
static uint32_t confirm_mpd(bufstream_tt *bs, uint32_t numbytes)
{
...
static uint32_t copybytes_mpd(bufstream_tt *bs, uint8_t *ptr, uint32_t numbytes)
{
    struct impl_stream* p = bs->Buf_IO_struct;
    uint32_t n;
    if(p->idx)
...
static uint32_t chunksize_mpd(bufstream_tt *bs)
{
    return bs->Buf_IO_struct->chunk_size;
}

static void free_mpd(bufstream_tt *bs)
{
    if(bs->Buf_IO_struct)
        bs->done(bs,0);
...
static uint32_t auxinfo_mpd(bufstream_tt *bs, uint32_t offs, uint32_t info_ID, void *info_ptr, uint32_t info_size)
{
    uint64_t *ptr;
    switch(info_ID)
    {
...
static uint32_t MC_EXPORT_API split_mpd(bufstream_tt *bs)
{
    struct impl_stream* p = bs->Buf_IO_struct;
...
static void MC_EXPORT_API done_mpd(bufstream_tt *bs, int32_t abort)
{
    struct impl_stream* p = bs->Buf_IO_struct;
...
static int32_t init_bs_mpd(bufstream_tt *bs, uint32_t bufsize)
{
    ...
    bs->usable_bytes = usable_bytes_mpd;
    bs->request      = request_mpd;
    bs->confirm      = confirm_mpd;
    bs->copybytes    = copybytes_mpd;
    bs->split        = split_mpd;
    bs->chunksize    = chunksize_mpd;
    bs->free         = free_mpd;
    bs->auxinfo      = auxinfo_mpd;
    bs->done         = done_mpd;
    bs->drive_ptr    = NULL;
    bs->drive        = NULL;
...
int32_t init_file_buf_mpd(bufstream_tt *bs,
