...
#ifdef _BS_UNICODE
int32_t init_file_buf_write_append(bufstream_tt *bs,
                                   const wchar_t* bs_filename,
                                   uint32_t bufsize, 
                                   void (*DisplayError)(char *txt))
#else
...
#endif
{
  ...
  bs->Buf_IO_struct->io = 
      ...
      CreateFile(bs_filename, 
 #endif
                 GENERIC_WRITE | FILE_APPEND_DATA, FILE_SHARE_READ, NULL, CREATE_ALWAYS,
                 FILE_ATTRIBUTE_NORMAL,
                 NULL);
  if(bs->Buf_IO_struct->io == INVALID_HANDLE_VALUE)
#else
 #ifdef _BS_UNICODE
   bs->Buf_IO_struct->io = _wfopen(bs_filename, L"ab");
 #else
  #if !defined(__QNX__)
   bs->Buf_IO_struct->io = fopen(bs_filename, "ab");
  #else
   bs->Buf_IO_struct->io = fopen64(bs_filename, "ab");
  #endif
 #endif
  if(!bs->Buf_IO_struct->io)
#endif
...
#ifdef _BS_UNICODE
bufstream_tt *open_file_buf_write_append(const wchar_t* bs_filename,
                                         uint32_t bufsize,
                                         void (*DisplayError)(char *txt))
#else
bufstream_tt *open_file_buf_write_append(const char *bs_filename,
                                         uint32_t bufsize,
                                         void (*DisplayError)(char *txt))
#endif
{
  ...
  if(p)
  {
    if(BS_OK != init_file_buf_write_append(p, bs_filename, bufsize, DisplayError))
    {
      free(p);
      p = NULL;
    }
