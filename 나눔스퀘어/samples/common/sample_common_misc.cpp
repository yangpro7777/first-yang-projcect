...
void statistics_printf(context_t context, const char *fmt, ...) {
    va_list va;
...
const char * license_file_path = NULL;
int32_t get_license_data(context_t context, void *pData)
{
    ...
    {
        FILE * pFile = fopen(license_file_path, "rb");
        ...
        if (pFile)
        {
            if (!fseek(pFile, 0, SEEK_END))
            {
                ...
                if ((readBytes = ftell(pFile)) > 0 && readBytes <= UINT_MAX)
                {
                    rsize = (uint32_t)readBytes;
                    if (rsize && pData) {
                        rewind(pFile);

                        size_t read_size = fread(pData, sizeof(uint8_t), rsize, pFile);
                    }
                    else
                    {
                        fclose(pFile);
...
extern "C" {
    void* mmalloc(context_t context, size_t size)
    {
        return malloc(size);
    }

    void mfree(context_t context, void* ptr)
    {
        free(ptr);
    }

    void* mrealloc(context_t context, void* ptr, size_t size)
    { 
...
void init_callbacks(callbacks_t& callbacks)
{
    ...
    callbacks.err_printf = error_printf;
    callbacks.prg_printf = progress_printf;
    callbacks.wrn_printf = warn_printf;
    callbacks.inf_printf = info_printf;
    callbacks.stat_printf = statistics_printf;
    callbacks.get_license_data = get_license_data;

    callbacks.malloc = mmalloc;
    callbacks.free = mfree;
    callbacks.realloc = mrealloc;
}
//! [resource functions dispatcher]
    ...
    while (!feof(fp))
    {
        ...
        switch (id)
        {
            case ID_FMT_:
            {
                ...
                uint8_t wavx_flag = 0;
                if(!fread(&audio_format, sizeof(uint16_t), 1, fp))
                {
                    fseek(fp, 0, 0);
                    return 0;
                }
                ...
                size_t reads = fread(&param->num_channels, sizeof(uint16_t), 1, fp);
                reads += fread(&param->sample_rate, sizeof(uint32_t), 1, fp);
                reads += fread(&param->bytes_per_sec, sizeof(uint32_t), 1, fp);
                reads += fread(&param->block_align, sizeof(uint16_t), 1, fp);
                reads += fread(&param->bits_per_sample, sizeof(uint16_t), 1, fp);
                if (reads != 5) 
                {
                    fseek(fp, 0, 0);
                    return 0;
...
const char* path2real(const char* path, char* buffer, size_t buffer_size)
{
    ...
    snprintf(buffer, buffer_size, "%s/Documents/%s", root, path);
    return buffer;
#else
    return path;
#endif
...
{
    return const_cast<char*>(path2real(const_cast<const char*>(path), buffer, buffer_size));
}
