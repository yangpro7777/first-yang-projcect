/********************************************************************
 Created: 2009/07/30
 File name: sample_demux_mp2_file_ds.cpp
...
// Name:             sample_demux_mp2_file_ds
// Used components:  demux_mp2_ds, render_fileindex_ds
// Description:      Demux all elementary streams from muxed MPEG-2 stream into target directory
// Command line:     sample_demux_mp2_file_ds.exe <INPUT_FILE> <OUTPUT_DIRECTORY>
// Expected result:  The tool will use file source filter for INPUT_FILE.
//                   It will connect MPEG-2 Demuxer filter. If the connection fails, the tool will abort
//                   ("Connection failed, not an MPEG-2 file?"). If the connection succeeded, a dump filter
//                   will be connected to all output pins of the demuxer.
//                   Filename for dump filters is "OUTPUT_DIRECTORY/pin_x.pin_type".
// Example with console output > sample_demux_mp2_file_ds.exe c:\input.mpg c:\demuxed_files\
// Created files:    c:\demuxed_files\pin_1.avc
//                   c:\demuxed_files\pin_2.aac
//                or c:\demuxed_files\pin_2.ac3
...
// Codec SDK headers
#include "demux_mp2_guid.h" // our MPEG-2 demuxer
#include "render_fileindex_guid.h" // our Sink filter
#include "aactypes_guid.h"

#define _WIN32_DCOM  // for CoInitializeEx()

#ifndef MEDIASUBTYPE_AMR
    ...
    if( outPin )
    {
        ...
        if( SUCCEEDED(outPin->QueryPinInfo( &info )) )
        {
            ...
            if( wcsstr( info.achName, L"aac" ) )
                ...
                extension = L"avc";
            else if( wcsstr( info.achName, L"h265" ) || wcsstr( info.achName, L"h.265" ) || wcsstr( info.achName, L"hevc" ) )
                extension = L"hevc";
            ...
            else if( MEDIATYPE_Audio == mt.majortype )
            {
                if( 0 == _wcsicmp( extension, defext ) )
                {
                    if( MEDIASUBTYPE_PCM == mt.subtype || MEDIASUBTYPE_DVD_LPCM_AUDIO == mt.subtype )
                        ...
                        extension = L"ac3";
                    else if( MEDIASUBTYPE_AAC == mt.subtype || MEDIASUBTYPE_LATMAAC == mt.subtype )
                        extension = L"aac";
...
int main( int argc, char * argv[] )
{
    ...
    {
        printf_s("\n==== MainConcept 'demux_mp2_file' DirectShow Sample ====\n"
                 "Usage: sample_demux_mp2_file_ds -i \"C:\\input.mpg\" -o \"C:\\demuxed_files\"\n"
                 "Supported input stream formats are: Muxed MPEG-2 streams\n"
                 "Options: -i <input_file> -o <output_directory>\n\n"
                );

        printf_s( "Please enter the input file (hit <Return> to abort):\n" );
        std::wcin.get( wInputFile, _MAX_PATH );
        std::wcin.ignore(std::cin.rdbuf()->in_avail());
        if( wcslen(wInputFile) <= 0 )
            return 0;

        printf_s( "Please enter the output directory name - be aware that existing files will be overwritten without prompt (hit <Return> to abort):\n" );
    ...
    // load MPEG-2 demultiplexer
    hr = CoCreateInstance( CLSID_MPGDMX, NULL, CLSCTX_INPROC_SERVER,
                           IID_IBaseFilter, reinterpret_cast<void**>(&pDemux) );
    if( SUCCEEDED(hr) )
        hr = pFilterGraph->AddFilter( pDemux, NULL );

    if( FAILED(hr) )
        printf_s( "\nERROR: Failed to add the MPEG-2 demultiplexer to the filter graph, possibly not registered!\n" );
    ...
    if( FAILED(hr) )
    {
        printf_s( "\nERROR: Failed to connect the source filter with the MPEG-2 demuxer, the input file is not MPEG-2!\n" );
        goto CLEAN_EXIT;
    }
    ...
    if( !pOutPinArray[0] )
    {
        printf_s( "\nERROR: Failed to find any MPEG-2 demuxer output pins!\n" );
        goto CLEAN_EXIT;
    }
    else
